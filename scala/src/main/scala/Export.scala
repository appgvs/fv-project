...
def merge[A : HOL.equal](s1: USet.USeta[(A, Nat.nat)],
...
def max_timestamp[A](s: USet.USeta[(A, Nat.nat)]): Nat.nat =
  Lattices_Big.Max[Nat.nat](Set.image[(A, Nat.nat),
                                       Nat.nat](((a: (A, Nat.nat)) =>
          Product_Type.snd[A, Nat.nat](a)),
         USet.query[(A, Nat.nat)](s)))

def insert[A : HOL.equal](l: USet.USeta[(A, Nat.nat)], e: A):
      USet.USeta[(A, Nat.nat)]
  =
  USet.add[(A, Nat.nat)](l, (e, Nat.plus_nat(max_timestamp[A](l), Nat.one_nat)))
